...
export async function generateCounterClauses(documentId: number, userId: number): Promise<void> {
  ...
  await db.execute(sql`UPDATE ${analysisResults} SET counter_clauses_status = 'processing' WHERE id = ${analysis.id}`);
  persistNow();
  ...
  try {
    ...
    if (risky.length === 0) {
      await db.execute(sql`UPDATE ${analysisResults} SET counter_clauses_status = 'skipped' WHERE id = ${analysis.id}`);
      persistNow();
...
- Return only JSON`,
        ...
        if (rewritten) {
          await db.execute(sql`UPDATE ${clauses} SET
    ...
    await db.execute(sql`UPDATE ${analysisResults} SET counter_clauses_status = 'ready' WHERE id = ${analysis.id}`);
    persistNow();
    emitToUser(userId, 'analysis:counter_clauses_ready', { documentId, status: 'ready', count: risky.length });
  } catch (err) {
    await db.execute(sql`UPDATE ${analysisResults} SET counter_clauses_status = 'failed' WHERE id = ${analysis.id}`);
    persistNow();
